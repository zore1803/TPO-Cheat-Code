  ...
  try {
    ...
    try {
      ...
      const response = await model.invoke([
        ["system", "You are an AI assistant that finds MCQ questions, programming questions, or other academic questions in text and provides detailed answers. First check whether the question has lettered answer options (A, B, C, D, etc) - if it does, it is an MCQ regardless of whether it also contains code, and you must provide ONLY the option letter(s), nothing else - no answer text, no code, no explanations. For a single question respond with just the letter, e.g. 'A'. For multiple questions, respond with one letter per line, e.g. 'A' newline 'B' newline 'C'. Only when there are NO lettered options should you treat it as a plain programming question and provide ONLY the code solution without any explanations or additional text. For other questions with no options, provide concise and accurate answers. If no relevant questions are found, respond with 'No relevant questions found.'"],
        ["user", text]
      ]);
...
const finalans = await solverModel.invoke([
         ["system", "You are an AI assistant that finds MCQ questions, programming questions, or other academic questions in text and provides detailed answers. First check whether the question has lettered answer options (A, B, C, D, etc) - if it does, it is an MCQ regardless of whether it also contains code, and you must provide ONLY the option letter(s), nothing else - no answer text, no code, no explanations. For a single question respond with just the letter, e.g. 'A'. For multiple questions, respond with one letter per line, e.g. 'A' newline 'B' newline 'C'. Only when there are NO lettered options should you treat it as a plain programming question and provide ONLY the code solution without any explanations or additional text - format code to work with modern code editors that have smart indentation features, each new line starting at the appropriate indentation level. For other questions with no options, provide concise and accurate answers. If no relevant questions are found, respond with 'No relevant questions found.'"],
         ["user", aiAnswers]
      ]);
